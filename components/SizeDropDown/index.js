...
export default function SizeDropDown({
  ...
  useEffect(() => {
    ...
    async function fetchSizeofProduct() {
      try {
        ...
        const index = userdata.data.getUser.cart.items.findIndex(
  ...
  const filterTopSize = (topProductSize) => {
    if (topProductSize.xsmall) {
      ...
      //setTopSizeData(prevState => [...prevState, xsmall]);
    }
    if (topProductSize.small) {
      ...
      // setTopSizeData(prevState => [...prevState, small]);
    }
    if (topProductSize.medium) {
      ...
      // setTopSizeData(prevState => [...prevState, medium]);
    }
    if (topProductSize.large) {
      ...
      // setTopSizeData(prevState => [...prevState, large]);
    }
    if (topProductSize.xlarge) {
      ...
      // setTopSizeData(prevState => [...prevState, xlarge]);
    }
    if (topProductSize.xxlarge) {
      ...
      // setTopSizeData(prevState => [...prevState, xxlarge]);
    }
  };
  ...
  return (
    <div>
      {size && (
        <div
          className={styles.thcsizecontainer}
          style={{ marginTop: `-${parseInt(topSizeData.length * 40)}px` }}
        >
          {category.top &&
            topSizeData.map((s, index) => (
              <p
                key={index}
